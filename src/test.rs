...
use cloudkit_proto::{
    base64_encode, CloudKitRecord, CloudKitValue, CuttlefishSerializedKey, ZoneRetrieveRequest,
};
use hkdf::Hkdf;
use icloud_auth::{AppleAccount, LoginState};
use keystore::{
    init_keystore,
    software::{NoEncryptor, SoftwareEncryptor, SoftwareKeystore},
};
...
use rustpush::{
    authenticate_apple, authenticate_smsless,
    cloud_messages::{CloudMessagesClient, MESSAGES_SERVICE},
    cloudkit::{
        record_identifier, CloudKitClient, CloudKitContainer, CloudKitSession, CloudKitState,
        DeleteRecordOperation, FetchZoneOperation, ZoneDeleteOperation, ZoneSaveOperation,
    },
    facetime::{FTClient, FTMember, FTMessage, FTState, FACETIME_SERVICE, VIDEO_SERVICE},
    findmy::{
        BeaconNamingRecord, FindMyClient, FindMyState, FindMyStateManager, MULTIPLEX_SERVICE,
    },
    get_gateways_for_mccmnc,
    keychain::{CloudKey, KeychainClient, KeychainClientState, KEYCHAIN_ZONES},
    login_apple_delegates,
    macos::MacOSConfig,
    name_photo_sharing::{
        IMessageNameRecord, IMessageNicknameRecord, IMessagePosterRecord, ProfilesClient,
    },
    passwords::{PasswordManager, PasswordState, SHARED_PASSWORDS_SERVICE},
    pcs::{PCSKey, PCSPrivateKey},
    posterkit::{
        PhotoPosterContentsFrame, PosterType, SimplifiedIncomingCallPoster, SimplifiedPoster,
        SimplifiedTranscriptPoster, TranscriptDynamicUserData,
    },
    prepare_put, register,
    sharedstreams::{
        round_seconds, AssetDetails, AssetFile, AssetMetadata, CollectionMetadata,
        FFMpegFilePackager, FileMetadata, FilePackager, PreparedAsset, PreparedFile,
        SharedStreamClient, SharedStreamsState, SyncController, SyncState,
    },
    statuskit::{StatusKitClient, StatusKitState, StatusKitStatus},
    APSConnectionResource, APSState, Attachment, CircleClientSession, CircleServerSession,
    CompactECKey, ConversationData, DebugMutex, DebugRwLock, EntitlementAuthState, FileContainer,
    IDSNGMIdentity, IDSUser, IDSUserIdentity, IMClient, IdmsAuthListener, IdmsMessage,
    IndexedMessagePart, KeyedArchive, LoginDelegate, MMCSFile, Message, MessageInst, MessageParts,
    MessageType, NormalMessage, PushError, RelayConfig, ShareProfileMessage, SharedPoster,
    TokenProvider, UpdateProfileMessage, MADRID_SERVICE,
};
...
fn read_file<T: Read + Seek, R: DeserializeOwned>(
...
fn read_archive<T: Read + Seek, R: DeserializeOwned>(
    archive: &mut ZipArchive<T>,
    path: &str,
) -> Result<R, PushError> {
    let mut manifest = vec![];
...
pub fn parse_poster(poster: &IMessagePosterRecord) -> Result<String, PushError> {
    ...
    let suggestion: Value = read_archive(
        &mut archive,
        "configuration/com.apple.posterkit.provider.identifierURL.suggestionMetadata.plist",
    )
    .unwrap_or(Value::Dictionary(Dictionary::new()));
    let complication: Value = read_archive(
        &mut archive,
        "configuration/versions/0/com.apple.posterkit.provider.instance.complicationLayout.plist",
    )
    .unwrap_or(Value::Dictionary(Dictionary::new()));
    ...
    // animoji
    let color_variations: Value = read_file(
        &mut archive,
        "configuration/versions/com.apple.posterkit.provider.instance.colorVariations.plist",
    )
    .unwrap_or(Value::Dictionary(Dictionary::new()));

    // image only
    let color_variations2: Value = read_archive(
        &mut archive,
        "configuration/versions/0/com.apple.posterkit.provider.instance.colorVariations.plist",
    )
    .unwrap_or(Value::Dictionary(Dictionary::new()));
    ...
    let model: Value = read_archive(
        &mut archive,
        "configuration/versions/0/contents/ConfigurationModel.plist",
    )
    .unwrap_or(Value::Dictionary(Dictionary::new()));
    let style: Value = read_file(
        &mut archive,
        "configuration/versions/0/contents/CB3D69CB-A1D0-4497-9105-9C6341A21BBB/style.plist",
    )
    .unwrap_or(Value::Dictionary(Dictionary::new()));
...
async fn handle_record(
    mut record: IMessageNicknameRecord,
    client: &IMClient,
    photo: &ProfilesClient<DefaultAnisetteProvider>,
    existing: &ShareProfileMessage,
) {
    if let Some(profile) = record.poster {
        ...
        fs::write(
            format!("posters/{stamp}/image.heif"),
            profile.low_res_poster,
        )
        .await
        .unwrap();
        fs::write(format!("posters/{stamp}/data.zip"), profile.package)
            .await
            .unwrap();
        fs::write(format!("posters/{stamp}/meta.plist"), profile.meta)
            .await
            .unwrap();
        fs::write(
            format!("posters/{stamp}/file.plist"),
            parse_poster(&profile_2).unwrap(),
        )
        .await
        .unwrap();
        ...
        to_poster.poster.r#type = PosterType::TranscriptDynamic {
            data: TranscriptDynamicUserData {
                identifier: "aurora_1".to_string(),
            },
        };
        ...
        client
            .send(&mut MessageInst::new(
                ConversationData {
                    participants: vec!["mailto:tag3@copper.jjtech.dev".to_string()],
                    cv_name: None,
                    sender_guid: None,
                    after_guid: None,
                },
                "mailto:tag3@copper.jjtech.dev",
                Message::UpdateProfile(UpdateProfileMessage {
                    profile: Some(existing.unwrap()),
...
async fn main() {
    ...
    let data: String = match fs::read_to_string("config.plist").await {
        ...
        Err(e) => match e.kind() {
            io::ErrorKind::NotFound => {
                let _ = fs::File::create("config.plist")
                    .await
    ...
    let gsa: GSAConfig = if let Ok(config) = plist::from_file("gsa.plist") {
        ...
        GSAConfig {
            ...
            pass: sha256(password.trim().as_bytes()).to_vec().into(),
        }
    };

    plist::to_file_xml("gsa.plist", &gsa).unwrap();

    let config: Arc<MacOSConfig> = Arc::new(
        if let Ok(config) = plist::from_file("hwconfig.plist") {
            config
        } else {
            ...
            let validation_data = general_purpose::STANDARD
                .decode(validation_data_b64.trim())
                .unwrap();
    ...
    let mut anisette_client = default_provider(
        config.get_gsa_config(&*connection.state.read().await, false),
        PathBuf::from_str("anisette_test").unwrap(),
    );
    ...
    let mut users = if let Some(state) = saved_state.as_ref() {
        ...
        .unwrap();
        let result = account
            .login_email_pass(&gsa.user, gsa.pass.as_ref())
            .await
            .unwrap();
        ...
        if let LoginState::NeedsDevice2FA = result {
            let mut s = CircleClientSession::new(dsid, done.clone(), connection.get_token().await)
                .await
                .unwrap();
            ...
            loop {
                ...
                if let Some(test) = listener.handle(msg.clone()).unwrap() {
                    ...
                    match test {
                        ...
                        IdmsMessage::RequestedSignIn(_) => info!(
                            "requested sign in code {}",
                            anisette_client
                                .lock()
                                .await
                                .provider
        ...
        let delegates = login_apple_delegates(
            &gsa.user,
            &pet,
            spd["adsid"].as_string().unwrap(),
            None,
            &mut *anisette_client.lock().await,
            config.as_ref(),
        ...
        let sharedstreams = SharedStreamsState::new(
            spd["DsPrsId"].as_unsigned_integer().unwrap().to_string(),
            &mobileme,
        );
        ...
        let trustedpeers = KeychainClientState::new(
            spd["DsPrsId"].as_unsigned_integer().unwrap().to_string(),
            spd["adsid"].as_string().unwrap().to_string(),
            &mobileme,
        );
        ...
        let cloudkitstate =
            CloudKitState::new(spd["DsPrsId"].as_unsigned_integer().unwrap().to_string());
    ...
    let identity = saved_state
        .as_ref()
        .map(|state| state.identity.clone())
        .unwrap_or(IDSNGMIdentity::new().unwrap());
    ...
    *state.lock().unwrap() = Some(SavedState {
    ...
    fs::write(
        "config.plist",
        plist_to_string(state.lock().unwrap().as_ref().unwrap()).unwrap(),
    )
    .await
    .unwrap();
    ...
    if let Some(mut s) = session {
        let mut subscription = connection.messages_cont.subscribe();
        s.setup_trusted_peers(keychain.clone(), b"antifa")
            .await
            .unwrap();
        let listener = IdmsAuthListener::new(connection.clone()).await;
